  ...
  return (
    <ul className="customMd:mt-24 mt-20 customMd:flex">
      {items.map((item) => (
        <div
          key={item.id}
          className={`mt-4 p-8 customMd:mt-0 customMd:px-14 customMd:py-12 ${item.id === 1 ? 'customMd:pl-0' : ''} ${item.id === 2 ? 'border border-gray-200' : ''}
        ...
        >
          ...
          <p className="mt-6 text-[15px] text-customGray">{item.description}</p>
        </div>
      ))}
    </ul>
  );
